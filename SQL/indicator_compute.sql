...
SELECT C.ISO3 AS CountryISO3, C.Name AS CountryName, Pop.Year AS Year, Pop.Value AS Population, GDP.Value AS GDP, PGap.Value AS PovertyGap, ROUND(PLine.Value, 0) AS PovertyLine, ROUND((PGap.Value * PLine.Value * Pop.Value) / 1000000, 0) AS CostClosingRPoverty, ROUND(((PGap.Value * PLine.Value * Pop.Value) / 1000000) / GDP.Value, 3) AS CostClosingRPovertyPercGDP
FROM WB_Countries C
